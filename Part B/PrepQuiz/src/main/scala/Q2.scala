object Q2 {
  ...
  def findSame(x: Any , l: List[Any]) : Boolean = {
    if (l.isEmpty) false;
    else if (l.head == x) true;
    else findSame(x , l.tail);
  }

  def countSame(l1:List[Any],l2:List[Any]) : Int = {
    if (l1.isEmpty) 0
    else (if (findSame(l1.head , l2)) 1 else 0) + countSame(l1.tail , l2)
  }
  def moreThanHalf(l1:List[Any],l2:List[Any]):Boolean ={
    countSame(l1,l2) > (l1.length / 2)
  }
